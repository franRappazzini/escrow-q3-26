use {
    ...
    escrowq32026::{Clock, SolanaSysvar},
    litesvm::LiteSVM,
    litesvm_token::{
...
fn test_take_fail_timestamp() {
    ...
    // Derive the PDA for the escrow account using the maker's public key and a seed value
    let escrow = Pubkey::find_program_address(
        &[b"escrow", maker.as_ref(), &1234u64.to_le_bytes()],
        &escrowq32026::id(),
    )
    .0;
    msg!("Escrow PDA: {}\n", escrow);
    ...
    let expiration = clock.unix_timestamp + 1000;
    ...
    let escrow_data =
        escrowq32026::state::Escrow::try_deserialize(&mut escrow_account.data.as_ref()).unwrap();
    assert_eq!(escrow_data.seed, 1234u64);
    assert_eq!(escrow_data.maker, maker);
    assert_eq!(escrow_data.mint_a, mint_a);
    assert_eq!(escrow_data.mint_b, mint_b);
    assert_eq!(escrow_data.receive, 10_000_000);
    ...
    // manual clock manipulation to fails the tx
    ...
    // Send the transaction and capture the result
    let tx = program.send_transaction(transaction);

    match tx {
        Ok(_) => panic!("The ix must fail"),
        Err(e) => {
            assert!(
                e.meta
                    .logs
                    .iter()
                    .any(|log| log.contains("The escrow has expired")),
                "Expected logs to contain `The escrow has expired`, got: {:?}",
                e.meta.logs
            );

            msg!("\n\n✅ Take transaction failed successfully");
        }
    }
}
